...
function set_file_prefix(file_url_prefixes)
{
    ...
    var audio_file_prefix = '../karaoke/' + file_url_prefixes;
    var   cdg_file_prefix = '../karaoke/' + file_url_prefixes;
...
$(document).ready(function(){
    ...
    var clone = '';
    flag = 0;
    $('#queue-select li').click(function(){
        set_file_prefix($(this).text());
        // console.log($(this).text());
    })
    $('#cdg_border').dblclick(function(){
        toggle_2x_mode();
        if(!flag){
            $('#queue-select').css('display','none');
            flag = 1;
            $('.video-play').css('border','none');
            console.log(clone);
        }else {
            $('#queue-select').css('display','block');
            $('.video-play').css('border','5px solid #171613');
            $('#cdg_audio').css('width','100%');
            flag = 0;
        }
